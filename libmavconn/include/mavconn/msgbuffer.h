...
struct MsgBuffer {
		...
		pos(0)
	{
		...
		obj.serialize(map); //this is where obj's MSG_ID, LENGTH copied to msg.msgid, msg.len, and obj's payload fields copied to msg's payload area. the obj is a derived Message type does that part. e.x, SET_MODE class is derived from mavlink::Message
//		std::cout << "msg.msgid: "<< msg.msgid <<" magic: "<<int(msg.magic)<<" len: "<<int(msg.len)<<"\n now call mavlink_finalize_message_buffer() to fill the header, this will decide mavlink version 1.0 or 2.0\n see /opt/ros/melodic/include/mavlink/v2.0/mavlink_helpers.h\n";
		mavlink::mavlink_finalize_message_buffer(&msg, sysid, compid, status, mi.min_length, mi.length, mi.crc_extra);  // this fill in other fields for the msg
//		std::cout<< "MsgBuffer: " << int(obj.get_message_info().id) <<" " << int(obj.get_message_info().length) << "\n";
//		std::cout << "msg.msgid: "<< msg.msgid <<" magic: "<<int(msg.magic)<<" len: "<<int(msg.len)<<"\n";
		len = mavlink::mavlink_msg_to_send_buffer(data, &msg);
//		std::cout << "msg.msgid: "<< msg.msgid <<" magic: "<< int(msg.magic)<<" len: "<<int(msg.len)<<"\n";
		// paranoic check, it must be less than MAVLINK_MAX_PACKET_LEN
