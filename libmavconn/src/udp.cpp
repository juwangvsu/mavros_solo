...
void MAVConnUDP::send_message(const mavlink_message_t *message)
{
	...
	//std::cout<<"send_message: magic, len, msgid: "<<int(message->magic)<<" "<<int(message->len)<<" "<<message->msgid<<"\n";
...
{
	if (message.get_name()=="TIMESYNC" | message.get_name()=="HEARTBEAT" | message.get_name()=="SYSTEM_TIME"){
	       //	std::cout<<" *** TIMESYNC MSG***\n";
	}
	else{
		std::cout<<std::flush<<"src/udp.c send_message "<< message.get_name()<<"\n\n";
		std::cout<<std::flush<<"src/udp.c m_mavlink_status: "<< int(get_status().flags) << " v1-bit: "<< int(get_status().flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1)<<"\n";
		std::cout<<std::flush<<"src/udp.c m_parse_status: "<< int(get_status_p()->flags) << " v1-bit: "<<int(get_status_p()->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1)<<"\n";
	}
	//CONSOLE_BRIDGE_logError(PFXd "send: channel closed!", conn_id);
	if (!is_open()) {
		CONSOLE_BRIDGE_logError(PFXd "send: channel closed!", conn_id);
...

	{
		...
		if (tx_q.size() >= MAX_TXQ_SIZE)
			throw std::length_error("MAVConnUDP::send_message: TX queue overflow");
		mavlink::mavlink_status_t tmp_status;
		tmp_status = get_status(); //this func get the mavlink_status which has a flag to remeber what mavlink version to use. the original code use get_status_p(), which is not workinig. see readme for more explaination.
		tx_q.emplace_back(message, &tmp_status, sys_id, source_compid);
		//jwang hack: 
		//tx_q.emplace_back(message, get_status_p(), sys_id, source_compid);
		// this call MsgBuffer constructor, which convert message (a drived mavlink::Message) to mavlink_message_t which contain the ready to send frame buffer, field filled out according to mavlink message format
		//
	}
	io_service.post(std::bind(&MAVConnUDP::do_sendto, shared_from_this(), true));
}
...
void MAVConnUDP::do_sendto(bool check_tx_state)
{
	...
	//std::cout<<"do_sendto buf_ref " << buf_ref<<"\n";
