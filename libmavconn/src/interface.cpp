...
void MAVConnInterface::parse_buffer(const char *pfx, uint8_t *buf, const size_t bufsize, size_t bytes_received)
{
	...
	for (; bytes_received > 0; bytes_received--) {
		...
		auto msg_received = static_cast<Framing>(mavlink::mavlink_frame_char_buffer(&m_buffer, &m_parse_status, c, &message, NULL)); // this is where msg from solo are processed. but it did not affect the outgoing mavlink flag. so we use 
		...
		if (msg_received != Framing::incomplete) {
			...
			//std::cout<<"src/interface.cpp parse_buffer() received a msg, m_parse_status.flags "<< int(m_parse_status.flags) << " "<<int(m_mavlink_status.flags)<<"\n";
			//std::cout<< "mavlink_message_t.magic, seq, len, msgid: " << int(message.magic) <<" " << int(message.seq) << " "<<int(message.len) << " " <<int(message.msgid)<<"\n";
...
void MAVConnInterface::log_recv(const char *pfx, mavlink_message_t &msg, Framing framing)
{
	...
	/*
	CONSOLE_BRIDGE_logWarn("%s%zu: recv: %s %4s Message-Id: %u [%u bytes] IDs: %u.%u Seq: %u",
			pfx, conn_id,
			proto_version_str,
			framing_str,
			msg.msgid, msg.len, msg.sysid, msg.compid, msg.seq);
...
{
	try {
		//std::cout<<"src/interface.cpp send_message_ignore_drop\n";
...
void MAVConnInterface::set_protocol_version(Protocol pver)
{
	if (pver == Protocol::V10)
		...
		m_mavlink_status.flags &= ~(MAVLINK_STATUS_FLAG_OUT_MAVLINK1);
	int flag = m_mavlink_status.flags;
	std::cout<<flag;
	std::cout<<std::flush<<"src/interface.c m_mavlink_status: "<< int(m_mavlink_status.flags);
	std::cout<< " v1-bit: "<<int(m_mavlink_status.flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1)
		<<"\n";
	std::cout<<std::flush<<"src/interface.c m_parse_status: "<< int(m_parse_status.flags) << " v1-bit: "<< int(m_parse_status.flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1)<<"\n";
